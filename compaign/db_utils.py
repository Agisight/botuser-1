...
async def new_compaign():
    ...
    async with pool.acquire() as conn:
        result = await conn.fetch("SELECT * FROM bot_compaign WHERE is_done = FALSE limit 1")
        if result:
            return result[0]
        return result
...
async def update_compaign(compaign_id, **kwargs):
    ...
    sql_query = "UPDATE bot_compaign SET %s WHERE id = $%d" % (sql_str[0:-1], var_number)
...
async def get_compaign(compaign_id):
    async with pool.acquire() as conn:
        return await conn.fetch('SELECT * FROM bot_compaign WHERE id = $1', compaign_id)
...
async def get_part_users(bot, offset, limit):
    ...
    async with pool.acquire() as conn:
        return await conn.fetch("SELECT * FROM bot_botuser "
                                "WHERE bot_id = $1 "
...
async def get_activity(bot, user):
    ...
    async with pool.acquire() as conn:
        result = await conn.fetch("SELECT COUNT(*) FROM bot_messagelog "
...
async def get_users_for_delay(bot, received_compaign, delay):
    ...
    async with pool.acquire() as conn:
        return await conn.fetch("SELECT * FROM bot_botuser "
                                "WHERE bot_id = $1 "
                                "AND id NOT IN (SELECT id FROM bot_botuser WHERE phone = any($2::varchar[])) "
